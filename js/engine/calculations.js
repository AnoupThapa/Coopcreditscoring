...
function runCalculations(data) {
    ...
    // COGS = raw_milk_purchase_cost (principal input cost)
    const inventory_days = data.raw_milk_purchase_cost > 0
        ? (data.average_inventory / data.raw_milk_purchase_cost) * 365
        : 0;
    ...
    // ── C8: Milk Loss % — (MilkLoss / TotalMilkCollected) * 100 ────────────────
    const milk_loss_pct = data.total_milk_collected_liters > 0
        ? (data.milk_loss_liters_during_collection / data.total_milk_collected_liters) * 100
        : 0;

    // ── C9: Yield Loss % — (MilkLoss / ProducedMilkModelB) * 100 ───────────────
    // (mirrors Data!D25 / Data!D26 from sheet)
    const yield_loss_pct = data.produced_milk_model_b_liters > 0
        ? (data.milk_loss_liters_during_collection / data.produced_milk_model_b_liters) * 100
        : 0;
    ...
    // ── C12: Payment Fairness % — 1 - (MaxDPD / 30) ────────────────────────────
    const payment_fairness = Math.max(0, 1 - (data.max_dpd_days / 30));

    // ── C13: Logistics Reliability — storage_availability_flag ─────────────────
    const logistics_reliability = data.storage_availability_flag;
    ...
    // ── C26: Quality SOP (numeric) ───────────────────────────────────────────────
    const quality_sop_numeric = data.quality_sop_numeric;

    // ── C27: % Sales via Bank — BankSales / TotalSales * 100 ─────────────────────
    const bank_sales_pct = data.total_sales > 0
        ? (data.bank_sales / data.total_sales) * 100
        : 0;
